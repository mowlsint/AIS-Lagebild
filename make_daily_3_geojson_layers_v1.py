...
Erzeugt 3 NICHT-ÜBERLAPPENDE GeoJSON-Dateien für uMap-Layer (ein/ausblendbar):

1) shadowfleet
   - Match gegen watchlist_shadowfleet.csv (MMSI/IMO)

2) ru_mid273
   - MMSI beginnt mit 273 (Heuristik "RU-Flagge"), aber NICHT shadowfleet

3) from_russia_ports
   - Schiff war im Lookback in einem RU-Hafen-BBox (Baltic + Arctic),
     UND hat am Zieldatum Punkte in Nordsee/Südliche Ostsee,
     aber NICHT shadowfleet und NICHT MID273.

Outputs (in --outdir, default exports):
...
Beispiel:
  python make_daily_3_geojson_layers_v1.py --in "logs_v4\\bbox_*.jsonl" --date 2025-12-21 --tz UTC --shadowfleet watchlist_shadowfleet.csv --outdir exports --lookback-days 14
"""
...
# Main monitoring area (min_lon, min_lat, max_lon, max_lat)
NORTHSEA = (-6.0, 50.0, 10.5, 62.0)
SOUTHBALTIC = (8.5, 53.3, 20.5, 56.2)

# RU port boxes (refined Kaliningrad/Baltiysk to avoid Bay of Gdansk false positives)
...
def main():
    ...
    for mmsi, pts in day_tracks.items():
        ...
        if shadow_hit:
            ...
            feats_shadow.extend(build_track_and_last(pts_sorted, props))
            continue

        if is_mid273(mmsi):
            c_mid273 += 1
            props = {**base_props, "layer": "ru_mid273", "ru_mid273": True, "method": "MMSI MID=273 (heuristic flag) excluding shadowfleet"}
            feats_mid273.extend(build_track_and_last(pts_sorted, props))
            continue
        ...
        if mmsi in last_ru_seen:
            ...
            feats_fromru.extend(build_track_and_last(pts_sorted, props))
    ...
    print(f"[layers] date={args.date} tz_used={tz_used} range_utc={day_start_utc.isoformat()}..{day_end_utc.isoformat()}")
    print(f"[layers] scanned_lines={scanned} kept_day_lines={kept_day} ru_port_hits={ru_hits}")
    print(f"[layers] ships: shadowfleet={c_shadow}, ru_mid273={c_mid273}, from_russia_ports={c_fromru}")
